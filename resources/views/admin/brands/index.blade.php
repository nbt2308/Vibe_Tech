            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-4 mb-8">
                ...
                <div class="bg-white overflow-hidden shadow rounded-xl border-l-4 border-green-500 hover:shadow-md transition-all duration-300">
                    <div class="p-5 flex items-center">
                        ...
                        <div>
                            <dt class="text-sm font-medium text-gray-500 truncate">Thương hiệu đang hoạt động</dt>
                            <dd class="text-2xl font-bold text-gray-900">{{ $brands_status_true }}</dd>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow rounded-xl border-l-4 border-blue-500 hover:shadow-md transition-all duration-300">
                    <div class="p-5 flex items-center">
                        ...
                        <div>
                            <dt class="text-sm font-medium text-gray-500 truncate">Thương hiệu có sản phẩm</dt>
                            <dd class="text-2xl font-bold text-gray-900">{{ $brands_linked_products }}</dd>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow rounded-xl border-l-4 border-gray-500 hover:shadow-md transition-all duration-300">
                    <div class="p-5 flex items-center">
                        ...
                        <div>
                            <dt class="text-sm font-medium text-gray-500 truncate">Thương hiệu không hoạt động</dt>
                            <dd class="text-2xl font-bold text-gray-900">{{ $brands_status_false }}</dd>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button
                                    ...
                                    <span>
                                        @if(request('status') === '1') Đang hoạt động @elseif(request('status') === '0') Ngừng hoạt động
                                        @else Trạng thái @endif
                                    </span>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['status' => null]) }}">Tất
                                    cả</x-dropdown-link>
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['status' => '1']) }}">Đang hoạt
                                    động</x-dropdown-link>
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['status' => '0']) }}">Ngừng hoạt
                                    động</x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
                    </div>
                        ...
                        <x-dropdown align="center" width="32">
                            <x-slot name="trigger">
                                <button class="w-full inline-flex items-center justify-between px-4 py-2 bg-white border border-gray-300 rounded-lg font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none transition duration-150">
                                    <span class="truncate">{{ request('per_page') }} thương hiệu</span>
                                    <i class="ml-2 fas fa-chevron-down text-[10px]"></i>
                                </button>
                            </x-slot>
                            <x-slot name="content">
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['per_page' => 5]) }}">5 thương hiệu</x-dropdown-link>
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['per_page' => 10]) }}">10 thương hiệu</x-dropdown-link>
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['per_page' => 20]) }}">20 thương hiệu</x-dropdown-link>
                                <x-dropdown-link href="{{ request()->fullUrlWithQuery(['per_page' => 50]) }}">50 thương hiệu</x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
                    </div>
                </div>
                                        ...
                                        <td class="py-3 px-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div
                                                    ...
                                                    <img src="{{ asset('storage/' . $brand->logo) }}" alt="Ảnh thương hiệu"
                                                        class="w-full h-full object-cover">
                                                </div>
                                                <div>
                                                    <p class="font-semibold text-gray-900 line-clamp-1">{{ $brand->name }}
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-3 px-4 max-w-[200px] md:max-w-xs overflow-hidden">
                                            <span class="line-clamp-1" title="{{ $brand->description }}">{{ $brand->description }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="px-2 py-1 font-bold text-md whitespace-nowrap">{{ $brand->products_count }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            @if($brand->status === 1)
                                                <span class="px-2 py-1 rounded-full text-[10px] font-bold bg-green-100 text-green-700 uppercase">{{ $brand->formatted_status }}</span>
                                            @else
                                                <span class="px-2 py-1 rounded-full text-[10px] font-bold bg-red-100 text-red-700 uppercase">{{ $brand->formatted_status }}</span>
                                            @endif
                                        </td>
                                        <td class="py-3 px-4 text-right space-x-3 whitespace-nowrap">
                                            <button x-data
                                                @click="$dispatch('open-modal', 'edit-brand-modal-{{ $brand->id }}');"
                                                class="text-blue-600 hover:bg-blue-50 rounded-lg transition-colors p-2"
                                                title="Sửa">
                                                <i class="fas fa-edit"></i></button>
                                            <button x-data
                                                @click="$dispatch('open-modal', 'delete-brand-modal-{{ $brand->id }}');"
                                    ...
                                    <tr>
                                        <td colspan="5" class="py-20 text-center">
                                            <div class="flex flex-col items-center justify-center text-gray-400">
                                                ...
                                                <p class="italic">Không tìm thấy thương hiệu nào phù hợp.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
