<div>
    <div id="modal-edit-{{ $brand->id }}" class="hidden">
        <div
            ...
            <div class="w-full max-w-lg bg-white shadow-lg rounded-lg p-8 relative z-10">
                <div class="flex items-center">
                    <h3 class="text-blue-600 text-xl font-semibold flex-1">Cập nhật thương hiệu</h3>
                    <svg onclick="closeEditModal({{ $brand->id }})" xmlns="http://www.w3.org/2000/svg"
                ...
                <form class="space-y-6 mt-8" action="{{ route('admin.brands.update', $brand->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div>
                        <label class="text-slate-900 text-sm mb-2 block">Tên Thương Hiệu</label>
                        <input type="text" placeholder="(Ví dụ: Razer, Logitech, Corsair,...)" name="name" required value="{{  $brand->name }}"
                    ...
                    <div>
                        ...
                        <textarea placeholder='(Ví dụ: Thương hiệu chuyên về gaming, Thương hiệu chuyên về văn phòng,...)' name="description" required
                            class="px-4 py-3 bg-gray-100 w-full text-slate-900 text-sm border-none focus:outline-blue-600 focus:bg-transparent rounded-lg"
                    ...
                    <div class="flex gap-4 !mt-8">
                        ...
                        <button onclick="closeEditModal({{ $brand->id }})" type="button"
