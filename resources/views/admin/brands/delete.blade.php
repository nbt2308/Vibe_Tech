<x-my-modal name="delete-brand-modal-{{ $brand->id }}" maxWidth="lg">
    <x-slot name="title">
        Xoá thương hiệu
    </x-slot>
    ...
    <x-slot name="content">
        <div class="text-center p-4">
            ...
            <p class="text-gray-700 text-lg mb-2">Bạn có chắc chắn muốn xóa thương hiệu này?</p>
            <p class="text-gray-900 font-bold text-xl mb-4">"{{ $brand->name }}"</p>
            <p class="text-sm text-gray-500">Hành động này không thể hoàn tác. Mọi dữ liệu liên quan đến thương hiệu sẽ bị
                xóa vĩnh viễn.</p>
        </div>

        <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="get"
            id="deleteForm-{{ $brand->id }}">
            @csrf
            @method('DELETE')
        </form>
    </x-slot>
    ...
    <x-slot name="footer">
        ...
        <button onclick="document.getElementById('deleteForm-{{ $brand->id }}').submit()" type="button"
