<x-my-modal name="delete-product-modal-{{ $product->id }}" maxWidth="lg">
    <x-slot name="title">
        Xoá sản phẩm
    </x-slot>
    ...
    <x-slot name="content">
        <div class="text-center p-4">
            ...
            <p class="text-gray-700 text-lg mb-2">Bạn có chắc chắn muốn xóa sản phẩm này?</p>
            <p class="text-gray-900 font-bold text-xl mb-4">"{{ $product->name }}"</p>
            <p class="text-sm text-gray-500">Hành động này không thể hoàn tác. Mọi dữ liệu liên quan đến sản phẩm sẽ bị xóa vĩnh viễn.</p>
        </div>

        <form action="{{ route('admin.products.destroy', $product->id) }}" method="get" id="deleteForm-{{ $product->id }}">
            @csrf
            @method('DELETE')
        </form>
    </x-slot>
    ...
    <x-slot name="footer">
        ...
        <button onclick="document.getElementById('deleteForm-{{ $product->id }}').submit()" type="button"
