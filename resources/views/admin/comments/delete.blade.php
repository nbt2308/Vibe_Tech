<x-my-modal name="delete-comment-modal-{{ $comment->id }}" maxWidth="lg">
    <x-slot name="title">
        Xoá bình luận
    </x-slot>
    ...
    <x-slot name="content">
        <div class="text-center p-4">
            ...
            <p class="text-gray-700 text-lg mb-2">Bạn có chắc chắn muốn xóa bình luận này?</p>
            <p class="text-gray-900 font-bold text-xl mb-4">"{{ $comment->comment_content }}"</p>
            <p class="text-sm text-gray-500">Hành động này không thể hoàn tác. Mọi dữ liệu liên quan đến bình luận này sẽ bị xóa vĩnh viễn.</p>
        </div>

        <form action="{{ route('admin.comments.destroy', $comment->id) }}" method="get" id="deleteForm-{{ $comment->id }}">
            @csrf
            @method('DELETE')
        </form>
    </x-slot>
