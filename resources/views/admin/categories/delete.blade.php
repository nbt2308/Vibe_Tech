<x-my-modal name="delete-category-modal-{{ $category->id }}" maxWidth="lg">
    <x-slot name="title">
        Xoá danh mục
    </x-slot>
    ...
    <x-slot name="content">
        <div class="text-center p-4">
            ...
            <p class="text-gray-700 text-lg mb-2">Bạn có chắc chắn muốn xóa danh mục này?</p>
            <p class="text-gray-900 font-bold text-xl mb-4">"{{ $category->name }}"</p>
            <p class="text-sm text-gray-500">Hành động này không thể hoàn tác. Mọi dữ liệu liên quan đến danh mục sẽ bị xóa vĩnh viễn.</p>
        </div>

        <form action="{{ route('admin.categories.destroy', $category->id) }}" method="get" id="deleteForm-{{ $category->id }}">
            @csrf
            @method('DELETE')
        </form>
    </x-slot>
    ...
    <x-slot name="footer">
        ...
        <button onclick="document.getElementById('deleteForm-{{ $category->id }}').submit()" type="button"
