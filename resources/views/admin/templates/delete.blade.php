<x-my-modal name="delete-template-modal-{{ $template->id }}" maxWidth="lg">
    <x-slot name="title">
        Xoá thuộc tính
    </x-slot>
    ...
    <x-slot name="content">
        <div class="text-center p-4">
            ...
            <p class="text-gray-700 text-lg mb-2">Bạn có chắc chắn muốn xóa thuộc tính này?</p>
            <p class="text-gray-900 font-bold text-xl mb-4">"{{ $template->display_name }}"</p>
            <p class="text-sm text-gray-500">Hành động này không thể hoàn tác. Mọi dữ liệu liên quan đến thuộc tính sẽ bị xóa vĩnh viễn.</p>
        </div>

        <form action="{{ route('admin.templates.destroy', $template->id) }}" method="get" id="deleteForm-{{ $template->id }}">
            @csrf
            @method('DELETE')
        </form>
    </x-slot>
    ...
    <x-slot name="footer">
        ...
        <button onclick="document.getElementById('deleteForm-{{ $template->id }}').submit()" type="button"
