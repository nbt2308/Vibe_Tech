<!-- Footer -->
<footer class="bg-dark text-gray-300 pt-12 pb-6 border-t-4 border-primary">
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 flex justify-between">
            <!-- Về chúng tôi -->
            <div>
                <a href="#" class="text-2xl font-bold text-white flex items-center gap-2 mb-4">
                    <i class="fas fa-microchip"></i>
                    Vibe Tech
                </a>
                <p class="text-sm text-gray-400 mb-4">Hệ thống bán lẻ linh kiện điện tử, gaming gear và phụ kiện máy
                    tính chính hãng, uy tín hàng đầu.</p>
                <div class="flex space-x-4">
                    <a href="#"
                        class="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-primary transition text-white"><i
                            class="fab fa-facebook-f"></i></a>
                    <a href="#"
                        class="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-primary transition text-white"><i
                            class="fab fa-youtube"></i></a>
                    <a href="#"
                        class="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center hover:bg-primary transition text-white"><i
                            class="fab fa-instagram"></i></a>
                </div>
            </div>

            <!-- Danh mục -->
            <div class="flex flex-col items-center">
                <h3 class="text-white font-bold text-lg mb-4">Danh Mục</h3>
                <ul class="space-y-2 text-sm">
                    @foreach ($categories as $category)
                        <li><a href="{{ route('categories.show', $category->slug) }}"
                                class="hover:text-primary transition">{{ $category->name }}</a></li>
                    @endforeach
                </ul>
            </div>

            <!-- Liên hệ -->
            <div>
                <h3 class="text-white font-bold text-lg mb-4">Thông Tin Liên Hệ</h3>
                <ul class="space-y-3 text-sm">
                    <li class="flex items-start gap-3">
                        <i class="fas fa-map-marker-alt mt-1 text-primary"></i>
                        <span>18, đường Ung Văn Khiêm, phường Long Xuyên, tỉnh An Giang</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i class="fas fa-phone-alt text-primary"></i>
                        <span>1900 1234 (8h00 - 21h30)</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i class="fas fa-envelope text-primary"></i>
                        <span>support@vibetech.vn</span>
                    </li>
                </ul>
            </div>
        </div>

        <!-- Copyright -->
        <div
            class="border-t border-gray-800 pt-6 mt-6 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
            <p>&copy; 2026 Vibe Tech. Tất cả quyền được bảo lưu.</p>
            <div class="flex space-x-2 mt-4 md:mt-0 text-2xl">
                <i class="fab fa-cc-visa hover:text-white transition"></i>
                <i class="fab fa-cc-mastercard hover:text-white transition"></i>
                <i class="fab fa-cc-paypal hover:text-white transition"></i>
            </div>
        </div>
    </div>
</footer>

